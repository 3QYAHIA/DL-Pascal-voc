...
class VOCDataset(VOCDetection):
    ...
    def __init__(self, root, year="2012", image_set="train", transform=None, download=False):
        ...
        # Hard code the limitation to 500 images
        max_samples = 500
        all_indices = list(range(len(self.images)))
        np.random.shuffle(all_indices)
        self.images = [self.images[i] for i in all_indices[:max_samples]]
        self.annotations = [self.annotations[i] for i in all_indices[:max_samples]]
